...
def agent_loop() -> None:
    ...
    store = SessionStore(agent_id="yjy")
    guard = ContextGuard()
...
"""
好问题！它们确实都面临同样的问题，但策略不同。让我对比一下：

  同样的问题，不同的解法

  所有 Agent 都逃不开这个核心矛盾：

  上下文窗口是有限的 (200K tokens)
    vs
  对话会越来越长 + 工具输出可能很大

  Claude Code 怎么做的

  Claude Code 用了四层防线，比 claw0 的两层更精细：

  第 0 层：尽量别塞进去（claw0 没有）

  这是 Claude Code 最关键的设计 —— 按需加载，不预装：

  ❌  笨办法: 把整个代码库都塞进上下文
     → 一个中等项目就 200K+ tokens, 直接爆了

  ✅ Claude Code: 初始只给最小上下文, 需要什么再读什么
     → 用户: "帮我修复 bug"
     → Claude Code: 先搜索相关文件 → 只读相关的那几行
     → 上下文始终保持精简

  claw0 的 s01/s02 是"用户说什么就往 messages 里塞什么"，没有这个意识。

  第 1 层：工具输出预截断（和 claw0 类似但更精细）

  claw0:    MAX_TOOL_OUTPUT = 50000 字符, 粗暴一刀切
  Claude Code:
              Read 工具: 默认最多 2000 行
              Grep 搜索: 限制结果数量
              Bash 输出: 超长自动截断 + [truncated] 标记
              → 每个工具有自己的合理限制

  第 2 层：自动压缩（和 claw0 类似但更聪明）
  ...
  第 3 层：手动压缩 + CLAUDE.md

  /compact                      → 手动触发压缩
  /compact focus on API重构      → 带焦点的压缩, 保留特定方向的信息
  CLAUDE.md 文件                → 压缩后不会丢失的关键上下文

  CLAUDE.md 是个很妙的设计 — 它是始终加载的，压缩不会丢：

  压缩前:
    messages = [CLAUDE.md, 聊了100轮, ...]  → 200K tokens

  压缩后:
    messages = [CLAUDE.md, "摘要: ...", 最近4轮]  → 50K tokens
                  ↑ 这部分永远在, 压缩不掉

  对比总结

  ┌────────────────────┬─────────────────────┬──────────────────────────────────┐
  │        策略        │    claw0 (教学)     │        Claude Code (生产)        │
  ├────────────────────┼─────────────────────┼──────────────────────────────────┤
  │ 预防: 按需加载     │ ❌ 没有  │ ✅        │不✅ 不预装代码库，需要时再读       │
  ├────────────────────┼─────────────────────┼──────────────────────────────────┤
  │ 预防: 工具输出截断 │ ✅  固定 50K 字符    │ ✅ 每个工具有独立限制             │
  ├────────────────────┼─────────────────────┼──────────────────────────────────┤
  │ 治疗: 自动压缩     │ ✅  同一个模型做摘要 │ ✅  用更便宜的模型 (Haiku) 做摘要 │
  ├────────────────────┼─────────────────────┼──────────────────────────────────┤
  │ 治疗: 手动压缩     │ ✅  /compact         │ ✅  /compact + 带焦点指令         │
  ├────────────────────┼─────────────────────┼──────────────────────────────────┤
  │ 持久上下文         │ ❌  没有             │ ✅  CLAUDE.md 压缩后不丢          │
  ├────────────────────┼─────────────────────┼──────────────────────────────────┤
  │ 溢出恢复           │ ✅  3 阶段重试       │ ✅  类似机制                      │
  └────────────────────┴─────────────────────┴──────────────────────────────────┘

  回到你的问题：Claude Code 是不是也有这个问题？

  当然有！ 压缩就是有损的，任何 Agent 都逃不开：

  真实场景:
    用户让 Claude Code 读了一个 3000 行的文件
    → Claude Code 只看到前 2000 行
    → bug 在第 2500 行
    → Claude Code 说 "没看到问题"
    → 用户: "你往下看看"
    → Claude Code: Read(file, offset=2000)
    → 这才看到 bug

  另一个场景:
    聊了 2 小时, Claude Code 自动压缩了
    → 1 小时前讨论的某个设计决定被摘要模糊了
    → Claude Code 可能做出和之前矛盾的决定
    → 用户: "你之前不是说用方案 A 吗?"
    → Claude Code: "抱歉, 我的摘要里没保留这个细节"

  这就是 Agent 的根本限制 — 上下文窗口有限，信息必然会丢失。各种策略只是在"什么时候丢"和"丢什么"上做文章。claw0 的实现是最简单直接的版本，Claude Code 在此基础上做了更多优化，但本质问题一样。
"""
